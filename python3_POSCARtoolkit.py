...
def interpret_car(lines):
    ...
    dict_car3 = dict(list(zip(list(dict_car2.values()), list(dict_car2.keys()))))   
...
def exe(*args):
    ...
    This POSCAR has Direct Coordinations, Conversion is starting....
            ...
            if  options.fixed and not  options.selected:    
                ...
                print('\n    -----------------------------------------------------------------------\n')
                print("    Cartesian Coordinates found, only for relaxing atoms!\n    Then atoms selected will be relaxed.") 
                print('    -----------------------------------------------------------------------\n')  
                convert(start_num,a=args)              
            else:
                print('\n    -----------------------------------------------------------')
    ...
    """)
            if options.fixed and not options.selected:
                ...
                print('\n    -----------------------------------------------------------')
                print("    No TTT OR FFF,please add them beforehand\n    Now converting %s to cartesian coordinates\n    Then type how many layers to be fixed, from bottom to top." %options.files)
                print('    -----------------------------------------------------------\n') 
...
def get_atoms(selects,dict_car2,dict_car): # return a list of atoms which user selected
    ...
    for i in selects:
        if '-' not in i:   # for example: atoms 1-5 
            ...
            elif i in list(dict_car2.keys()): # For Example: Ru C H O                                
                n_s, n_e = dict_car2.get(i).split('---')[:]
                ele_list.extend(list(range(int(n_s), int(n_e)+1))) 
            elif i == 'all':
                ele_list = list(range(1,sum([value for value in list(dict_car.values())])+1))
                break
            else:
            ...
            ele_list.extend(list(range(num_start, num_end)))
    return  ele_list 
